...
const Server = require('@phoscoder/whatsapp-cloud-api/dist/classes/server').default;
...
async function handleTextMessage(message, senderId) {
  ...
  if (text.includes('hello') || text.includes('hi')) {
    await messenger.send_message('Hello! How can I help you today?', senderId);
          ...
          sections: [
            {
              title: 'Services',
              rows: [
                { id: 'service_1', title: 'Service 1', description: 'Description for service 1' },
                { id: 'service_2', title: 'Service 2', description: 'Description for service 2' },
              ],
            },
          ],
        },
    ...
    await messenger.send_message(`You said: "${message.text.body}"`, senderId);
  }
}
...
async function handleImageMessage(message, senderId) {
  ...
  const mediaData = await messenger.get_media(mediaId);
  
  console.log('Media data:', mediaData);
  
  await messenger.send_message('Thank you for the image!', senderId);
}

// Handle video messages
async function handleVideoMessage(message, senderId) {
  console.log(`Video received from ${senderId}`);
  await messenger.send_message('Thank you for the video!', senderId);
}

// Handle audio messages
async function handleAudioMessage(message, senderId) {
  console.log(`Audio received from ${senderId}`);
  await messenger.send_message('Thank you for the audio message!', senderId);
}

// Handle document messages
async function handleDocumentMessage(message, senderId) {
  console.log(`Document received from ${senderId}`);
  await messenger.send_message('Thank you for the document!', senderId);
}
...
async function handleInteractiveMessage(message, senderId) {
  ...
  if (message.interactive.type === 'button_reply') {
    ...
    await messenger.send_message(`You selected button: ${buttonId}`, senderId);
  } else if (message.interactive.type === 'list_reply') {
    const listId = message.interactive.list_reply.id;
    const listTitle = message.interactive.list_reply.title;
    await messenger.send_message(`You selected: ${listTitle}`, senderId);
  }
}
